...
$(document).on('click', '.person-search-result', function(e){
    ...
    var OpenElement = '<div class="person-search-result-name">Nimi: '+ClickedPersonData.firstname+' '+ClickedPersonData.lastname+'</div> <div class="person-search-result-bsn">CSN: '+ClickedPersonData.citizenid+'</div> <div class="person-opensplit"></div> &nbsp; <div class="person-search-result-dob">Sünni Aasta: '+ClickedPersonData.birthdate+'</div> <div class="person-search-result-number">Telefoni NR: '+ClickedPersonData.phone+'</div> <div class="person-search-result-nationality">Rahvus: '+ClickedPersonData.nationality+'</div> <div class="person-search-result-gender">Sugu: '+Gender+'</div> &nbsp; <div class="person-search-result-apartment"><span id="'+ClickedPersonId+'">Korter: '+appartementData.label+'</span> <i class="fas fa-map-marker-alt appartment-adress-location" id="'+ClickedPersonId+'"></i></div> &nbsp; <div class="person-search-result-warned">Signaali Andnud: '+IsWarrant+'</div> <div class="person-search-result-driverslicense">Juhiluba: '+HasLicense+'</div>';
    ...
    if (OpenedPerson === null) {
        ...
        var PreviousElement = '<div class="person-search-result-name">Nimi: '+PreviousPersonData.firstname+' '+PreviousPersonData.lastname+'</div> <div class="person-search-result-bsn">CSN: '+PreviousPersonData.citizenid+'</div>';
...
$(document).on('click', '.house-adress-location', function(e){
    ...
    $.post('https://lc-phone/SetGPSLocation', JSON.stringify({
...
$(document).on('click', '.appartment-adress-location', function(e){
    ...
    $.post('https://lc-phone/SetApartmentLocation', JSON.stringify({
...
$(document).on('click', '.confirm-search-person-test', function(e){
    ...
    if (SearchName !== "") {
        $.post('https://lc-phone/FetchSearchResults', JSON.stringify({
            input: SearchName,
        }), function(result){
            if (result != null) {
                $(".person-search-results").html("");
...
$(document).on('click', '.confirm-search-person-house', function(e){
    ...
    if (SearchName !== "") {
        $.post('https://lc-phone/FetchPlayerHouses', JSON.stringify({
            input: SearchName,
        }), function(result){
            if (result != null) {
                $(".person-search-results").html("");
...
$(document).on('click', '.confirm-search-vehicle', function(e){
    ...
    if (SearchName !== "") {
        $.post('https://lc-phone/FetchVehicleResults', JSON.stringify({
            input: SearchName,
        }), function(result){
            if (result != null) {
                $(".vehicle-search-results").html("");
...
$(document).on('click', '.scan-search-vehicle', function(e){
    e.preventDefault();
    $.post('https://lc-phone/FetchVehicleScan', JSON.stringify({}), function(vehicle){
        if (vehicle != null) {
            $(".vehicle-search-results").html("");
            var APK = "Yes";
            if (!vehicle.status) {
...
$(document).on('click', '.meos-recent-alert', function(e){
    ...
    if (alertData != undefined){
        if (alertData.coords != undefined && alertData.coords != null) {
            $.post('https://lc-phone/SetAlertWaypoint', JSON.stringify({
                alert: alertData,
            }));
        } else {
