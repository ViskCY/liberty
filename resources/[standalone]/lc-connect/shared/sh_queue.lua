...
function Queue:DebugPrint(msg)
    if Queue.Debug then
        msg = "^3JÄRJEKORD: ^0" .. tostring(msg) .. "^7"
        print(msg)
    end
end
...
function Queue:AddToQueue(ids, connectTime, name, src, deferrals)
    ...
    for pos, data in ipairs(queueList) do
        if tmp.priority then
            ...
            if _pos then
                Queue:DebugPrint(string_format("%s[%s] prioritiseeriti ja paigutati %d/%d järjekorras", tmp.name, ids[1], _pos, queueCount))
                break
            end
        end
    end
    ...
    if not _pos then
        ...
        Queue:DebugPrint(string_format("%s[%s] pandi %d/%d järjekorda", tmp.name, ids[1], _pos, queueCount))
    end
...
function Queue:AddToConnecting(ids, ignorePos, autoRemove, done)
    local function remove()
        ...
        Queue:DebugPrint("Mängijat ei saanud ühenduste loendisse lisada")
    end
...
local function playerConnect(name, setKickReason, deferrals)
    ...
    Queue:CanJoin(src, function(reason)
        ...
        if reason then
            -- prevent joining
            allow = false
            done(reason and tostring(reason) or "Teie liitumine blokeeriti")
            Queue:RemoveFromQueue(ids)
            Queue:RemoveFromConnecting(ids)
    ...
    while true do
        ...
        if not data or not data.deferrals or not data.source or not pos then
            remove("[Järjekord] Järjekorrast eemaldatud, järjekorra andmed on kehtetud :(")
