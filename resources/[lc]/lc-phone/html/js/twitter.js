...
$(document).on('click', '.twitter-header-tab', function(e){
    ...
    if (PressedTwitterTab !== CurrentTwitterTab) {
        ...
        if (PressedTwitterTab === "twitter-mentions") {
            $.post('https://lc-phone/ClearMentions');
        }
...
QB.Phone.Notifications.LoadTweets = function(Tweets) {
    ...
    if (Tweets !== null && Tweets !== undefined && Tweets !== "" && Tweets.length > 0) {
        ...
        $.each(Tweets, function(i, Tweet){
            ...
            if (clean == '') clean = 'Hmm, ma ei tohiks seda teha...'
...
$(document).on('click','#twt-delete-click',function(e){
    ...
    $.post('https://lc-phone/DeleteTweet', JSON.stringify({id: source}))
})
...
$(document).on('click', '#send-tweet', function(e){
    ...
    if (TweetMessage != "") {
        ...
        $.post('https://lc-phone/PostNewTweet', JSON.stringify({
        ...
        QB.Phone.Notifications.Add("fab fa-twitter", "Twitter", "Kirjuta Siia!", "#1DA1F2");
    };
    $('#tweet-new-url').val("");
...
$(document).on('click', '.hashtag-tag-text', function(e){
    ...
    if (!HashtagOpen) {
        ...
        $.post('https://lc-phone/GetHashtagMessages', JSON.stringify({hashtag: Hashtag}), function(HashtagData){
