...
local function KnockDoorAnim(home)
    ...
    if home then
        ...
        exports['lc-core']:DrawText(Lang:t("info.other_dealers_button"), 'left')
        AwaitingInput()
    else
        TriggerServerEvent("InteractSound_SV:PlayOnSource", "knock_door", 0.2)
        Wait(100)
    ...
    if deliveryTimeout > 0 then
        ...
        QBCore.Functions.Progressbar("work_dropbox", Lang:t("info.delivering_products"), 3500, false, true, {
            ...
            if Config.UseTarget then
                exports['lc-target']:RemoveZone('drugDeliveryZone')
            else
    ...
    if Config.UseTarget then
        ...
        dealerCombo:onPlayerInOut(function(isPointInside)
            if isPointInside then
                if not dealerIsHome then
                    exports['lc-core']:DrawText(Lang:t("info.knock_button"),'left')
                    AwaitingInput()
                elseif dealerIsHome then
                    exports['lc-core']:DrawText(Lang:t("info.other_dealers_button"), 'left')
                    AwaitingInput()
    ...
    if Config.UseTarget then
        ...
        drugDeliveryZone:onPlayerInOut(function(isPointInside)
            if isPointInside then
                ...
                exports['lc-core']:DrawText(Lang:t("info.deliver_items_button", {itemAmount = activeDelivery["amount"], itemLabel = QBCore.Shared.Items[activeDelivery["itemData"]["item"]]["label"]}),'left')
                CreateThread(function()
