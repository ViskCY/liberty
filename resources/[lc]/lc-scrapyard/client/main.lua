        ...
        for k,v in pairs(Config.Locations[i]) do
            if k ~= 'main' then
                if Config.UseTarget then
                    if k == 'deliver' then
                        exports["lc-target"]:AddBoxZone("yard"..i, v.coords, v.length, v.width, {
                            name = "yard"..i,
                            heading = v.heading,
                            minZ = v.coords.z - 1,
                            maxZ = v.coords.z + 1,
                        ...
                        exports["lc-target"]:AddBoxZone("list"..i, v.coords, v.length, v.width, {
                            name = "list"..i,
                            heading = v.heading,
                            minZ = v.coords.z - 1,
                            maxZ = v.coords.z + 1,
                    ...
                    scrapCombo:onPlayerInOut(function(isPointInside)
                        if isPointInside then
                            if not isBusy then
                                if k == 'deliver' then
                                    exports['lc-core']:DrawText(Lang:t('text.disassemble_vehicle'),'left')
                                else
                                    exports['lc-core']:DrawText(Lang:t('text.email_list'),'left')
                                end
                                KeyListener(k)
                            end
                        else
