...
$(document).on('click', '.phone-app-footer-button', function(e){
    ...
    if (PressedFooterTab !== CurrentFooterTab) {
        ...
        if (PressedFooterTab == "recent") {
            $.post('https://lc-phone/ClearRecentAlerts');
            ...
            if (status.IsOnline) {
                if (status.CanCall) {
                    if (!status.InCall) {
                        ...
                        QB.Phone.Notifications.Add("fas fa-phone", "Telefon", "Oled Juba Kõnes!");
                    }
                } else {
                    QB.Phone.Notifications.Add("fas fa-phone", "Telefon", "See Isik On Hetkel Hõivatud!");
                }
            } else {
                QB.Phone.Notifications.Add("fas fa-phone", "Telefon", "See inimene pole saadaval!");
            }
        } else {
            QB.Phone.Notifications.Add("fas fa-phone", "Telefon", "Sa Ei Saa Ise Endale Helistada!");
        }
    });
});
        ...
        if (cData.number !== QB.Phone.Data.PlayerData.charinfo.phone) {
            if (status.IsOnline) {
                if (status.CanCall) {
                    ...
                    QB.Phone.Notifications.Add("fas fa-phone", "Telefon", "See Isik On Hõivatud!");
                }
            } else {
                QB.Phone.Notifications.Add("fas fa-phone", "Telefon", "See Isik Pole Hetkel Saadaval!");
            }
        } else {
            QB.Phone.Notifications.Add("fas fa-phone", "Telefon", "Sa Ei Saa Ise Endale Helistada!");
        }
...
QB.Phone.Functions.LoadContacts = function(myContacts) {
    ...
    if (myContacts !== null) {
        $.each(myContacts, function(i, contact){
            ...
            if (contact.name == '') contact.name = 'Hmm, ma ei tohiks seda teha...'
        ...
        $("#total-contacts").text(TotalContacts+ " Kontakti");
    } else {
        $("#total-contacts").text("0 Kontakti #Kurb");
    }
};
...
$(document).on('click', '#new-chat-phone', function(e){
    ...
    if (ContactData.number !== QB.Phone.Data.PlayerData.charinfo.phone) {
        $.post('https://lc-phone/GetWhatsappChats', JSON.stringify({}), function(chats){
        ...
        setTimeout(function(){
            ...
            $.post('https://lc-phone/GetWhatsappChat', JSON.stringify({phone: ContactData.number}), function(chat){
        ...
        QB.Phone.Notifications.Add("fa fa-phone-alt", "Telefon", "Sa Ei Saa Whatsappida Ise Endale..", "default", 3500);
    }
});
...
$(document).on('click', '#edit-contact', function(e){
    ...
    if (CurrentEditContactData.name == '') CurrentEditContactData.name = 'Hmm, ma ei tohiks seda teha...'
...
$(document).on('click', '#edit-contact-save', function(e){
    ...
    if (ContactName == '') ContactName = 'Hmm, ma ei tohiks seda teha...'
    var ContactNumber = $(".phone-edit-contact-number").val();
    var ContactIban = $(".phone-edit-contact-iban").val();

    if (ContactName != "" && ContactNumber != "") {
        $.post('https://lc-phone/EditContact', JSON.stringify({
        ...
        QB.Phone.Notifications.Add("fas fa-exclamation-circle", "Redigeeri Kontakti", "Täitke Kõik Väljad!");
    }
});
...
$(document).on('click', '#edit-contact-delete', function(e){
    ...
    $.post('https://lc-phone/DeleteContact', JSON.stringify({
        CurrentContactName: ContactName,
        CurrentContactNumber: ContactNumber,
        CurrentContactIban: ContactIban,
    }), function(PhoneContacts){
...
$(document).on('click', '.phone-keypad-key', function(e){
    ...
    if (!isNaN(PressedButton)) {
        ...
        if (ClearNumberTimer == null) {
            $("#phone-keypad-input").text("Puhastatud")
...
$(document).on('click', '#add-contact-save', function(e){
    ...
    if (ContactName != "" && ContactNumber != "") {
        $.post('https://lc-phone/AddNewContact', JSON.stringify({
        ...
        if (SelectedSuggestion !== null) {
            $.post('https://lc-phone/RemoveSuggestion', JSON.stringify({
            ...
            $(".amount-of-suggested-contacts").html(amount + " Kontakte");
        }
    } else {
        QB.Phone.Notifications.Add("fas fa-exclamation-circle", "Lisa Kontakt", "Täida Kõik Vormid");
    }
});
            ...
            if (status.IsOnline) {
                if (status.CanCall) {
                    if (!status.InCall) {
                        ...
                        QB.Phone.Notifications.Add("fas fa-phone", "Telefon", "Teil on juba kõne pooleli!");
                    }
                } else {
                    QB.Phone.Notifications.Add("fas fa-phone", "Telefon", "See inimene on juba kõnes!");
                }
            } else {
                QB.Phone.Notifications.Add("fas fa-phone", "Telefon", "See inimene pole saadaval!");
            }
        } else {
            QB.Phone.Notifications.Add("fas fa-phone", "Telefon", "Te ei saa helistada oma numbrile!");
        }
    });
}
...
$(document).on('click', '#outgoing-cancel', function(e){
    ...
    $.post('https://lc-phone/CancelOutgoingCall');
});

$(document).on('click', '#incoming-deny', function(e){
    e.preventDefault();

    $.post('https://lc-phone/DenyIncomingCall');
});

$(document).on('click', '#ongoing-cancel', function(e){
    e.preventDefault();

    $.post('https://lc-phone/CancelOngoingCall');
});
...
IncomingCallAlert = function(CallData, Canceled, AnonymousCall) {
    if (!Canceled) {
        if (!QB.Phone.Data.CallActive) {
            ...
            QB.Phone.Animations.TopSlideUp('.'+QB.Phone.Data.currentApplication+"-app", 400, -160);
            setTimeout(function(){
                var Label = "Teile saabus kõne kasutajalt "+CallData.name
                if (AnonymousCall) {
                    Label = "Sulle helistab anonüümne inimene"
                }
...
$(document).on('click', '#incoming-answer', function(e){
    ...
    $.post('https://lc-phone/AnswerCall');
});
...
QB.Phone.Functions.SetupSuggestedContacts = function(Suggested) {
    ...
    if (AmountOfSuggestions > 0) {
        ...
        $(".amount-of-suggested-contacts").html("0 Kontakte");
    }
}
