...
--     }
    ...
    exports['lc-menu']:showHeader(headerMenu)
end
    ...
    zone:onPlayerInOut(function (isPointInside)
        if isPointInside and text then
            if targetKey == "entrancePos" then
                ...
                exports['lc-core']:DrawText(text, 'left')
            end
        else
            if targetKey == "entrancePos" then
                CloseMenuFull()
...
local function SetInApartmentTargets()
    ...
    exports['lc-target']:AddBoxZone("ApartmentStash", vector3(Apartments.Locations[ClosestHouse].coords.enter.x - POIOffsets.stash.x, Apartments.Locations[ClosestHouse].coords.enter.y - POIOffsets.stash.y, Apartments.Locations[ClosestHouse].coords.enter.z - CurrentOffset + POIOffsets.stash.z), 1.2, 1.2, { 
        name = "ApartmentStash", 
        heading=270, 
        debugPoly = false, 
    ...
    exports['lc-target']:AddBoxZone("ApartmentClothing", vector3(Apartments.Locations[ClosestHouse].coords.enter.x - POIOffsets.clothes.x, Apartments.Locations[ClosestHouse].coords.enter.y - POIOffsets.clothes.y, Apartments.Locations[ClosestHouse].coords.enter.z - CurrentOffset + POIOffsets.clothes.z), 1.6, 1.6, { 
    ...
    -- exports['lc-target']:AddBoxZone("ApartMentLogout", vector3(Apartments.Locations[ClosestHouse].coords.enter.x - POIOffsets.logout.x, Apartments.Locations[ClosestHouse].coords.enter.y + POIOffsets.logout.y, Apartments.Locations[ClosestHouse].coords.enter.z - CurrentOffset + POIOffsets.logout.z), 1.6, 1.6, { 
    ...
    exports['lc-target']:AddBoxZone("ApartmentExit", vector3(Apartments.Locations[ClosestHouse].coords.enter.x - POIOffsets.exit.x, Apartments.Locations[ClosestHouse].coords.enter.y - POIOffsets.exit.y, Apartments.Locations[ClosestHouse].coords.enter.z - CurrentOffset + POIOffsets.exit.z), 1.6, 1.6, { 
        name = "ApartmentExit", 
        heading=270, 
        debugPoly = false, 
...
local function EnterApartment(house, apartmentId, new)
    ...
    QBCore.Functions.TriggerCallback('apartments:GetApartmentOffset', function(offset)
        if offset == nil or offset == 0 then
            QBCore.Functions.TriggerCallback('apartments:GetApartmentOffsetNewOffset', function(newoffset)
                ...
                local data = exports['lc-interior']:CreateApartmentFurnished(coords)
                Wait(100)
                HouseObj = data[1]
                POIOffsets = data[2]
                InApartment = true
    ...
    -- exports['lc-target']:AddBoxZone("ApartmentExit", vector3(-274.98, -961.39, -27.76), 1.6, 1.6, { 
    --     name = "ApartmentExit", 
    --     heading=270, 
    --     debugPoly = false, 
    --     minZ = Apartments.Locations[ClosestHouse].coords.enter.z - CurrentOffset + POIOffsets.exit.z-1, 
...
local function LeaveApartment(house)
    ...
    exports['lc-target']:RemoveZone("ApartmentStash")
    exports['lc-target']:RemoveZone("ApartmentClothing")
    exports['lc-target']:RemoveZone("ApartmentExit")
    -- exports['lc-target']:RemoveZone("ApartMentLogout")
end
...
function MenuOwners()
    QBCore.Functions.TriggerCallback('apartments:GetAvailableApartments', function(apartments)
        if next(apartments) == nil then
            ...
            exports['lc-menu']:openMenu(apartmentMenu)
        end
    end, ClosestHouse)
end
...
exports['lc-target']:AddBoxZone("getin", vector3(-270.95, -957.87, 31.22), 1, 3, {
    name="getin",
    heading=109.06,
    debugPoly=false,
    minZ=20.79,
