    ...
    window.addEventListener('message', function (event) {
        ...
        if (data.action == "ui") {
            ...
            if (data.toggle) {
                ...
                setTimeout(function(){
                    $.post('https://lc-multicharacter/setupCharacters');
                    setTimeout(function(){
                        ...
                        $.post('https://lc-multicharacter/removeBlur');
                    }, 2000);
                }, 2000);
            } else {
...
$('.disconnect-btn').click(function(e){
    ...
    $.post('https://lc-multicharacter/closeUI');
    $.post('https://lc-multicharacter/disconnectButton');
});
...
$(document).on('click', '.RegisterGreen', function (e) {
    ...
    if(success){
        $.post('https://lc-multicharacter/createNewCharacter', JSON.stringify({
            firstname: firstname,
            lastname: lastname,
...
$(document).on('click', '#accept-delete', function(e){
    $.post('https://lc-multicharacter/removeCharacter', JSON.stringify({
...
function RefreshExpertData() {
    ...
    $(".SelectChar-header").html("Vali Karakter");
    $('.BlackScreenForNotAbuse').fadeOut(150);
    $('#Expert-woman').attr('src','https://cdn.discordapp.com/attachments/877595408325574656/878254563063369818/female.png');
    $('#Expert-man').attr('src','https://cdn.discordapp.com/attachments/877595408325574656/878254579622502471/male.png');
...
$(document).on('click', '.SelectChar-SpawnChar', function(e) {
    ...
    if (selectedChar !== null) {
        $.post('https://lc-multicharacter/selectCharacter', JSON.stringify({
