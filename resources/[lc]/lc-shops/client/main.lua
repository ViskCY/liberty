...
local function createPeds()
    ...
    for k, v in pairs(Config.Locations) do
        ...
        if Config.UseTarget then
            exports['lc-target']:AddTargetEntity(ShopPed[k], {
                options = {
                    {
                        label = v["targetLabel"],
                        icon = v["targetIcon"],
    ...
    if Config.UseTarget then
        exports['lc-target']:AddTargetEntity(ShopPed["casino"], {
            options = {
                {
                    label = 'Sell Chips',
    ...
    CreateThread(function()
        ...
        combo:onPlayerInOut(function(isPointInside, _, zone)
            if isPointInside then
                ...
                exports['lc-core']:DrawText(Lang:t("info.open_shop"))
                listenForControl()
            else
        ...
        sellChips:onPlayerInOut(function(isPointInside)
            if isPointInside then
                inChips = true
                exports['lc-core']:DrawText(Lang:t("info.sell_chips"))
            else
                inChips = false
                exports['lc-core']:HideText()
            end
