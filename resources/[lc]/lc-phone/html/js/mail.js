...
$(document).on('click', '#accept-mail', function(e){
    ...
    $.post('https://lc-phone/AcceptMailButton', JSON.stringify({
...
$(document).on('click', '#remove-mail', function(e){
    e.preventDefault();
    var MailData = $("#"+OpenedMail).data('MailData');
    $.post('https://lc-phone/RemoveMail', JSON.stringify({
        mailId: MailData.mailid
    }));
    $(".mail-home").animate({
        left: 0+"vh"
...
QB.Phone.Functions.SetupMails = function(Mails) {
    ...
    $("#mail-header-mail").html(QB.Phone.Data.PlayerData.charinfo.firstname+"."+QB.Phone.Data.PlayerData.charinfo.lastname+"@libertycity.com");
    $("#mail-header-lastsync").html("Viimati Sünkroonitud "+MessageTime);
    if (Mails !== null && Mails !== undefined) {
        if (Mails.length > 0) {
            ...
            $(".mail-list").html('<p class="nomails">Sul Pole Ühtegi Maili..</p>');
        }

    }
}

var MonthFormatting = ["Jaanuar", "Veebruar", "Märts", "Aprill", "Mai", "Juuni", "Juuli", "August", "September", "Oktoober", "November", "Detsember"];
...
$(document).on('click','#new-advert-photo',function(e){
    e.preventDefault();
    $.post('https://lc-phone/TakePhoto',function(url){
        if(url){
...
$(document).on('click', '#new-advert-submit', function(e){
    ...
    if (Advert !== "") {
        ...
        if (!picture){
            $.post('https://lc-phone/PostAdvert', JSON.stringify({
                message: Advert,
                url: null
            }));
        }else {
        ...
        QB.Phone.Notifications.Add("fas fa-ad", "Reklaam", "Sa Ei Saa Reklaamida Tühjust!", "#ff8f1a", 2000);
    }
});
...
QB.Phone.Functions.RefreshAdverts = function(Adverts) {
    ...
    if (Adverts.length > 0 || Adverts.length == undefined) {
        ...
        $(".advert-list").html("");
        var element = '<div class="advert"><span class="advert-sender">Ühtegi Kuulutust Pole Hetkel!</span></div>';
        $(".advert-list").append(element);
    }
}
...
$(document).on('click','#adv-delete',function(e){
    e.preventDefault();
    $.post('https://lc-phone/DeleteAdvert', function(){
        setTimeout(function(){
            QB.Phone.Notifications.Add("fas fa-ad", "Reklaam", "Reklaam Kustutatud", "#ff8f1a", 2000);
        },400)
    });
})
