...
local function RegisterTraphouseEntranceTarget(traphouseID, traphouseData)
    ...
    exports['lc-target']:AddBoxZone(boxName, coords, boxData.length, boxData.width, {
        name = boxName,
...
local function RegisterTraphouseEntranceZone(traphouseID, traphouseData)
    ...
    zone:onPlayerInOut(function (isPointInside)
        if isPointInside then
            exports['lc-core']:DrawText('[E] ' .. Lang:t('targetInfo.enter'), 'left')
        else
...
local function RegisterTraphouseInteractionZone(traphouseID, traphouseData)
    ...
    zone:onPlayerInOut(function (isPointInside)
        if isPointInside then
            exports['lc-core']:DrawText('[E] ' .. Lang:t('targetInfo.options'), 'left')
        else
            exports['lc-core']:HideText()
...
local function RegisterTraphouseExitZone(coords, traphouseID, traphouseData)
    ...
    zone:onPlayerInOut(function (isPointInside)
        if isPointInside then
            exports['lc-core']:DrawText('[E] ' .. Lang:t("targetInfo.leave"), 'left')
        else
            exports['lc-core']:HideText()
        end
...
local function EnterTraphouse(data)
    ...
    data = exports['lc-interior']:CreateTrevorsShell(coords)
    TraphouseObj = data[1]
    POIOffsets = data[2]
...
local function LeaveTraphouse(k, data)
    ...
    DoScreenFadeOut(250)
    Wait(250)
    exports['lc-interior']:DespawnInterior(TraphouseObj, function()
    ...
    if Config.UseTarget then
        exports['lc-target']:RemoveZone('traphouseInteraction' .. k)
        data.polyzoneBoxData['interaction'].created = false

        exports['lc-target']:RemoveZone('traphouseExit' .. k)
