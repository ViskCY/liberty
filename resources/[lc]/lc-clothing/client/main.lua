...
Citizen.CreateThread(function()
    for k, v in pairs (Config.Stores) do
        if Config.Stores[k].shopType == "clothing" then
            ...
            AddTextComponentString("Riidepood")
        ...
        if Config.Stores[k].shopType == "barber" then
            ...
            BeginTextCommandSetBlipName("STRING")
            AddTextComponentString("Juuksur")
                ...
                if dist < 30 then
                    if not creatingCharacter then
                        ...
                        if dist < 3.5 then
                            ...
                            inZone  = true
                            if Config.Stores[k].shopType == "clothing" then 
                                text = '<b>Riidepood</b></p>Vajuta [E] et avada menüü'
                            elseif Config.Stores[k].shopType == "barber" then
                                text = '<b>Juuksur</b></p>Vajuta [E] et avada menüü'
                            elseif Config.Stores[k].shopType == "surgeon" then
                                text = '<b>Plastik Opp</b></p>Vajuta [E] et avada menüü'
                            end
...
RegisterNetEvent('doj:client:clothingMenu', function()
	exports['lc-menu']:openMenu({
		{
			header = "Riidepood",
			txt = "Maksa Sularahaga: $"..Config.Cost.ClothingCash,
            params = {
		...
		{
			header = "Riidepood",
			txt = "Maksa Pangaga: $"..Config.Cost.ClothingBank,
			params = {
				event = "doj:server:payForClothing",
                isServer = true,
                args = 2
...
RegisterNetEvent('doj:client:barberMenu', function()
	exports['lc-menu']:openMenu({
		{
			header = "Barber Shop",
			txt = "Maksa Sularahaga: $"..Config.Cost.BarberCash,
            params = {
		...
		{
			header = "Juuksur",
			txt = "Maksa Pangaga: $"..Config.Cost.BarberBank,
			params = {
				event = "doj:server:payForBarber",
                isServer = true,
                args = 2
...
RegisterNetEvent('doj:client:surgeonMenu', function()
	exports['lc-menu']:openMenu({
		{
			header = "Kirurg",
			txt = "Maksa Sularahaga: $"..Config.Cost.SurgeonCash,
            params = {
		...
		{
			header = "Kirurg",
			txt = "Maksa Pangaga: $"..Config.Cost.SurgeonBank,
			params = {
...
RegisterNetEvent('doj:client:accessShop', function(args)
    ...
    if args == 1 then 
        openMenu({
            ...
            {menu = "accessoires", label = "Peatarvikud", selected = false}
        
        })
    elseif args == 2 then 
        openMenu({
            {menu = "clothing", label = "Juuksed", selected = true},
        })
    elseif args == 3 then 
        openMenu({
            {menu = "clothing", label = "Funktsioonid", selected = true},
        })
    end
end)
                    ...
                    if not creatingCharacter then
                        ...
                        if dist < 2 then
                            ...
                            if PlayerData.job.name == Config.ClothingRooms[k].requiredJob then 
                                text = '<b>Vaata Riideid</b><p>Vajuta [E] et avada menüü</p>'
                                if IsControlJustPressed(0, 38) then -- E
                                    ...
                                    QBCore.Functions.TriggerCallback('qb-clothing:server:getOutfits', function(result)
                                        openMenu({
                                            ...
                                            {menu = "myOutfits", label = "Minu Riided", selected = false, outfits = result},
                                            {menu = "character", label = "Riided", selected = false},
                                            {menu = "accessoires", label = "Peatarvikud", selected = false}
                                        })
                                    end)
                                end
...
AddEventHandler('qb-clothing:client:openOutfitMenu', function()
    QBCore.Functions.TriggerCallback('qb-clothing:server:getOutfits', function(result)
        openMenu({
            {menu = "myOutfits", label = "Minu Riided", selected = true, outfits = result},
        })
    end)
end)
