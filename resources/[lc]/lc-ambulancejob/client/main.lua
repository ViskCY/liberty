...
RegisterNetEvent('hospital:client:RespawnAtHospital', function()
    ...
    if exports["lc-policejob"]:IsHandcuffed() then
...
CreateThread(function()
    while true do
        ...
        if isInHospitalBed and canLeaveBed then
            sleep = 0
            exports['lc-core']:DrawText(Lang:t('text.bed_out'))
...
if Config.UseTarget then
    CreateThread(function()
        for k, v in pairs(Config.Locations["checking"]) do
            exports['lc-target']:AddBoxZone("checking"..k, vector3(v.x, v.y, v.z), 3.5, 2, {
                name = "checking"..k,
                heading = -72,
                debugPoly = false,
                minZ = v.z - 2,
        ...
        for k, v in pairs(Config.Locations["beds"]) do
            -- exports['lc-target']:AddBoxZone("beds"..k,  v.coords, 2.5, 2.3, {
            --     name = "beds"..k,
            --     heading = -20,
            --     debugPoly = false,
            --     minZ = v.coords.z - 1,
    ...
    CreateThread(function()
        ...
        for k, v in pairs(Config.Locations["checking"]) do
            ...
            checkingCombo:onPlayerInOut(function(isPointInside)
                if isPointInside then
                    if doctorCount >= Config.MinimalDoctors then
                        exports['lc-core']:DrawText(Lang:t('text.call_doc'),'left')
                        CheckInControls("checkin")
                    else
                        exports['lc-core']:DrawText(Lang:t('text.check_in'), 'left')
                        CheckInControls("checkin")
                    end
                else
                    listen = false
        ...
        for k, v in pairs(Config.Locations["beds"]) do
            ...
            bedCombo:onPlayerInOut(function(isPointInside)
                if isPointInside then
                    exports['lc-core']:DrawText(Lang:t('text.lie_bed'), 'left')
