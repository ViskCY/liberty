...
local function DestroyPickupTarget()
  ...
  if Config.UseTarget then
    exports['lc-target']:RemoveZone(pickupTargetID)
    pickupZone = nil
  else
    pickupZone:destroy()
    pickupZone = nil
...
local function RegisterEntranceTarget()
  ...
  if Config.UseTarget then
    entranceZone = exports['lc-target']:AddBoxZone(entranceTargetID, coords, 1, 4, {
    ...
    entranceZone:onPlayerInOut(function(isPointInside)
      if isPointInside then
        exports['lc-core']:DrawText(Lang:t("text.point_enter_warehouse"), 'left')
      else
...
local function RegisterExitTarget()
  ...
  if Config.UseTarget then
    exitZone = exports['lc-target']:AddBoxZone(exitTargetID, coords, 1, 4, {
      name = exitTargetID,
      heading = 270,
    ...
    exitZone:onPlayerInOut(function(isPointInside)
      if isPointInside then
        exports['lc-core']:DrawText(Lang:t("text.point_exit_warehouse"), 'left')
      else
        exports['lc-core']:HideText()
      end
...
local function DestroyExitTarget()
  ...
  if Config.UseTarget then
    exports['lc-target']:RemoveZone(exitTargetID)
    exitZone = nil
  else
    exitZone:destroy()
    exitZone = nil
...
local function RegisterDutyTarget()
  ...
  if Config.UseTarget then
    dutyZone = exports['lc-target']:AddBoxZone(dutyTargetID, coords, 1, 1, {
      name = dutyTargetID,
      heading = 270,
    ...
    dutyZone:onPlayerInOut(function(isPointInside)
      if isPointInside then
        exports['lc-core']:DrawText(GetDutyTargetText(), 'left')
      else
        exports['lc-core']:HideText()
      end
...
local function DestroyDutyTarget()
  ...
  if Config.UseTarget then
    exports['lc-target']:RemoveZone(dutyTargetID)
    dutyZone = nil
  else
    dutyZone:destroy()
    dutyZone = nil
...
local function RegisterDeliveyTarget()
  ...
  if Config.UseTarget then
    deliveryZone = exports['lc-target']:AddBoxZone(deliveryTargetID, coords, 1, 1, {
    ...
    deliveryZone:onPlayerInOut(function(isPointInside)
      if isPointInside and carryPackage then
        exports['lc-core']:DrawText(Lang:t("text.point_hand_in_package"), 'left')
      else
        exports['lc-core']:HideText()
      end
...
local function DestroyDeliveryTarget()
  ...
  if Config.UseTarget then
    exports['lc-target']:RemoveZone(deliveryTargetID)
    deliveryZone = nil
  else
...
function RegisterPickupTarget(coords)
  ...
  if Config.UseTarget then
    pickupZone = exports['lc-target']:AddBoxZone(pickupTargetID, targetCoords, 4, 1.5, {
      name = pickupTargetID,
      heading = coords.h,
      minZ = coords.z - 1.0,
      maxZ = coords.z + 2.0,
    ...
    pickupZone:onPlayerInOut(function(isPointInside)
      if isPointInside then
        exports['lc-core']:DrawText( Lang:t("text.point_get_package"), 'left')
      else
        exports['lc-core']:HideText()
      end
