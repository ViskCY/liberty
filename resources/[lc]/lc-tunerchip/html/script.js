...
$(document).on('click', '#save', function(){
    $.post('https://lc-tunerchip/save', JSON.stringify({
...
$(document).on('click', "#save-neon", function(){
    if (RainbowNeon) {
        $.post('https://lc-tunerchip/saveNeon', JSON.stringify({
...
$(document).on('click', '#save-headlights', function(){
    $.post('https://lc-tunerchip/saveHeadlights', JSON.stringify({
        value: headlightVal,
...
$(document).on('click', '#save-stancer', function(){
    ...
    $.post('https://lc-tunerchip/SetStancer', JSON.stringify({
...
lcTuner.Open = function() {
    $.post('https://lc-tunerchip/checkItem', JSON.stringify({item: "tunerlaptop"}), function(hasItem){
        if (hasItem) {
...
lcTuner.Close = function() {
    $('.container').fadeOut(250);
    $.post('https://lc-tunerchip/exit');
}