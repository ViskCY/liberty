...
local function RobberyPed()
    if Config.UseTarget then
        exports['lc-target']:AddEntityZone('stealingPed', stealingPed, {
            name = 'stealingPed',
            debugPoly = false,
        }, {
            options = {
                {
                    ...
                    action = function()
                        ...
                        stealingPed = nil
                        stealData = {}
                        exports['lc-target']:RemoveZone('stealingPed')
                    end,
                ...
                if IsEntityDead(stealingPed) then
                    ...
                    if not Config.UseTarget and #(pos - pedpos) < 1.5 then
                        if not textDrawn then
                            textDrawn = true
                            exports['lc-core']:DrawText(Lang:t("info.pick_up_button"))
                        end
            ...
            if getRobbed <= Config.RobberyChance then
                ...
                if pedDist2 < 1.5 and cornerselling then
                    if Config.UseTarget and not zoneMade then
                        zoneMade = true
                        exports['lc-target']:AddEntityZone('sellingPed', ped, {
                            name = 'sellingPed',
                            debugPoly = false,
                        }, {
                            options = {
                                {
                                    ...
                                    action = function(entity)
                                        ...
                                        exports['lc-target']:RemoveZone('sellingPed')
                                        PoliceCall()
                                    end,
                                },
                                {
                    ...
                    elseif not Config.UseTarget then
                        if not textDrawn then
                            textDrawn = true
                            exports['lc-core']:DrawText(Lang:t("info.drug_offer", {bags = bagAmount, drugLabel = currentOfferDrug.label, randomPrice = randomPrice}))
                        end
