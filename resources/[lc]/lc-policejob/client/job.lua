...
function MenuImpound(currentSelection)
    ...
    QBCore.Functions.TriggerCallback("police:GetImpoundedVehicles", function(result)
        ...
        if shouldContinue then
            ...
            exports['lc-menu']:openMenu(impoundMenu)
        end
    end)
...
RegisterNetEvent('police:client:EvidenceStashDrawer', function(data)
    ...
    if #(pos - takeLoc) <= 1.0 then
        local drawer = exports['lc-input']:ShowInput({
...
if Config.UseTarget then
    CreateThread(function()
        ...
        for k, v in pairs(Config.Locations["duty"]) do
            exports['lc-target']:AddBoxZone("PoliceDuty_"..k, vector3(v.x, v.y, v.z), 1, 1, {
                name = "PoliceDuty_"..k,
                heading = 11,
                debugPoly = false,
                minZ = v.z - 5,
    ...
    dutyCombo:onPlayerInOut(function(isPointInside)
        if isPointInside then
            dutylisten = true
            if not onDuty then
                exports['lc-core']:DrawText(Lang:t('info.on_duty'),'left')
                dutylistener()
            else
                exports['lc-core']:DrawText(Lang:t('info.off_duty'),'left')
                dutylistener()
            end
        else
            inDuty = false
...
CreateThread(function()
    ...
    stashCombo:onPlayerInOut(function(isPointInside, _, _)
        if isPointInside then
            inStash = true
            exports['lc-core']:DrawText(Lang:t('info.stash_enter'), 'left')
            stash()
        else
            exports['lc-core']:HideText()
            inStash = false
    ...
    trashCombo:onPlayerInOut(function(isPointInside)
        if isPointInside then
            inTrash = true
            if onDuty then
                exports['lc-core']:DrawText(Lang:t('info.trash_enter'),'left')
                trash()
            end
        else
            inTrash = false
    ...
    fingerprintCombo:onPlayerInOut(function(isPointInside)
        if isPointInside then
            inFingerprint = true
            if onDuty then
                exports['lc-core']:DrawText(Lang:t('info.scan_fingerprint'),'left')
                fingerprint()
            end
        else
            inFingerprint = false
    ...
    armouryCombo:onPlayerInOut(function(isPointInside)
        if isPointInside then
            inAmoury = true
            if onDuty then
                exports['lc-core']:DrawText(Lang:t('info.enter_armory'),'left')
                armoury()
            end
        else
            inAmoury = false
    ...
    helicopterCombo:onPlayerInOut(function(isPointInside)
        if isPointInside then
            ...
            if onDuty then
                if IsPedInAnyVehicle(PlayerPedId(), false) then
                    exports['lc-core']:HideText()
                    exports['lc-core']:DrawText(Lang:t('info.store_heli'), 'left')
                    heli()
                else
                    exports['lc-core']:DrawText(Lang:t('info.take_heli'), 'left')
                    heli()
                end
            end
        else
    ...
    impoundCombo:onPlayerInOut(function(isPointInside, point)
        if isPointInside then
            ...
            if onDuty then
                if IsPedInAnyVehicle(PlayerPedId(), false) then
                    exports['lc-core']:DrawText(Lang:t('info.impound_veh'), 'left')
                    impound()
                else
    ...
    garageCombo:onPlayerInOut(function(isPointInside, point)
        if isPointInside then
            ...
            if onDuty and PlayerJob.name == 'police' then
                if IsPedInAnyVehicle(PlayerPedId(), false) then
                    exports['lc-core']:DrawText(Lang:t('info.store_veh'), 'left')
		    garage()
                else
                    local currentSelection = 0
...
RegisterNetEvent("qb-stash:personalpd")
AddEventHandler("qb-stash:personalpd", function()
    TriggerServerEvent("inventory:server:OpenInventory", "stash", "policestash_"..QBCore.Functions.GetPlayerData().citizenid)
    TriggerEvent("inventory:client:SetCurrentStash", "policestash_"..QBCore.Functions.GetPlayerData().citizenid)
end)
