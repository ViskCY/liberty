			...
			if k==random then
				CreateThread(function()
					while grapeZones[k].isInside and k==random do
						exports['lc-core']:DrawText(Lang:t("task.start_task"),'right')
				...
				while Zones[2].isInside do
					if not wineStarted then
						if not loadIngredients then
							exports['lc-core']:DrawText(Lang:t("task.load_ingrediants"),'right')
							...
							if not finishedWine then
								exports['lc-core']:DrawText(Lang:t("task.wine_process"),'right')
								if IsControlJustPressed(0, 38) and not LocalPlayer.state.inv_busy then
									StartWineProcess()
								end
							else
								exports['lc-core']:DrawText(Lang:t("task.get_wine"),'right')
								if IsControlJustPressed(0, 38) and not LocalPlayer.state.inv_busy then
						...
						exports['lc-core']:DrawText(Lang:t("task.countdown",{time=winetimer}),'right')
						Wait(999)
					end
					Wait(1)
				end
		...
		if not startVineyard and PlayerJob.name == "vineyard" then
			CreateThread(function()
				while Zones[3].isInside do
					exports['lc-core']:DrawText(Lang:t("task.make_grape_juice"),'right')
					if IsControlJustPressed(0, 38) and not LocalPlayer.state.inv_busy then
