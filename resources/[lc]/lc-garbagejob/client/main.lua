...
local function DeliverAnim()
    ...
    if Config.UseTarget and hasBag then
        ...
        exports['lc-target']:RemoveTargetEntity(garbageVehicle)
        if (amountOfBags - 1) <= 0 then
            ...
            exports['lc-target']:AddCircleZone('garbagebin', vector3(CL.coords.x, CL.coords.y, CL.coords.z), 2.0,{
...
function TakeAnim()
    ...
    QBCore.Functions.Progressbar("bag_pickup", Lang:t("info.picking_bag"), math.random(3000, 5000), false, true, {
        ...
        if Config.UseTarget and not hasBag then
            hasBag = true
            exports['lc-target']:RemoveZone("garbagebin")
            exports['lc-target']:AddTargetEntity(garbageVehicle, {
            options = {
                ...
                if not hasBag and canTakeBag then
                    if Distance < 1.5 then
                        if not GarbText then
                            GarbText = true
                            exports['lc-core']:DrawText(Lang:t("info.grab_garbage"), 'left')
                        end
                    ...
                    if DoesEntityExist(garbageVehicle) then
                        ...
                        if TruckDist < 2 then
                            if not TrucText then
                                TrucText = true
                                exports['lc-core']:DrawText(Lang:t("info.dispose_garbage"), 'left')
                            end
...
local function spawnPeds()
    ...
    for i = 1, #Config.Peds do
        ...
        if Config.UseTarget then
            exports['lc-target']:AddTargetEntity(ped, {
                options = {{type = "client", event = "qb-garbagejob:client:MainMenu", label = Lang:t("target.talk"), icon = 'fa-solid fa-recycle',}},
                distance = 2.0
            })
        else
            ...
            if options then
                ...
                zone:onPlayerInOut(function(inside)
                    if LocalPlayer.state.isLoggedIn then
                        if inside then
                            exports['lc-core']:DrawText(Lang:t("info.talk"), 'left')
                            Listen4Control()
                        else
                            ControlListen = false
                            exports['lc-core']:HideText()
...
RegisterNetEvent('qb-garbagejob:client:MainMenu', function()
    ...
    exports['lc-menu']:openMenu(MainMenu)
end)
