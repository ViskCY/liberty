...
$(document).on('click', '#accept-background', function(e){
    ...
    if (hasCustomBackground === false) {
        QB.Phone.Notifications.Add("fas fa-paint-brush", "Seaded", QB.Phone.Settings.Backgrounds[QB.Phone.Settings.Background].label+" is set!")
        ...
        QB.Phone.Notifications.Add("fas fa-paint-brush", "Seaded", "Isiklik taustakomplekt lisatud!")
        QB.Phone.Animations.TopSlideUp(".settings-"+QB.Phone.Settings.OpenedTab+"-tab", 200, -100);
        $(".phone-background").css({"background-image":"url('"+QB.Phone.Settings.Background+"')"});
    }

    $.post('https://lc-phone/SetBackground', JSON.stringify({
...
$(document).on('click', '#accept-profilepicture', function(e){
    ...
    if (ProfilePicture === "default") {
        QB.Phone.Notifications.Add("fas fa-paint-brush", "Seaded", "Standardne avatar lisatud!")
        QB.Phone.Animations.TopSlideUp(".settings-"+QB.Phone.Settings.OpenedTab+"-tab", 200, -100);
        $("[data-settingstab='profilepicture']").find('.settings-tab-icon').html('<img src="./img/default.png">');
    } else {
        QB.Phone.Notifications.Add("fas fa-paint-brush", "Seaded", "Isiklik avatar lisatud!")
        QB.Phone.Animations.TopSlideUp(".settings-"+QB.Phone.Settings.OpenedTab+"-tab", 200, -100);
        $("[data-settingstab='profilepicture']").find('.settings-tab-icon').html('<img src="'+ProfilePicture+'">');
    }
    $.post('https://lc-phone/UpdateProfilePicture', JSON.stringify({
