...
local function enterRobberyHouse(house)
    ...
    if Config.Houses[house]["tier"] == 1 then
        data = exports['lc-interior']:CreateHouseRobbery(coords)
    end
    Wait(100)
    houseObj = data[1]
    POIOffsets = data[2]
...
local function lockpickFinish(success)
    if success then
        --policecal = math.random(1, 3) == 1 or 2 or 3
...
local function searchCabin(cabin)
    ...
    QBCore.Functions.Progressbar("houserobbery", "Otsimine...", (7000), false, true, {
...
CreateThread(function()
    Wait(500)
    requiredItems = {
        [1] = {name = QBCore.Shared.Items["lockpick"]["name"], image = QBCore.Shared.Items["lockpick"]["image"]} or {name = QBCore.Shared.Items["advancedlockpick"]["name"], image = QBCore.Shared.Items["advancedlockpick"]["image"]},
