            ...
            if Player then
                ...
                if Player.PlayerData.job and payment > 0 and (QBShared.Jobs[Player.PlayerData.job.name].offDutyPay or Player.PlayerData.job.onduty) then
                    if QBCore.Config.Money.PayCheckSociety then
                        local account = exports['lc-management']:GetAccount(Player.PlayerData.job.name)
                        if account ~= 0 then -- Checks if player is employed by a society
                            if account < payment then -- Checks if company has enough money to pay society
                                TriggerClientEvent('QBCore:Notify', Player.PlayerData.source, Lang:t('error.company_too_poor'), 'error')
                            else
                                Player.Functions.AddMoney('bank', payment)
                                exports['lc-management']:RemoveMoney(Player.PlayerData.job.name, payment)
...
function QBCore.Functions.UseItem(source, item)
    ...
    exports['lc-inventory']:UseItem(source, item)
end

-- Kick Player

function QBCore.Functions.Kick(source, reason, setKickReason, deferrals)
    reason = '\n' .. reason .. '\n🔸 vaata meie discordi: ' .. QBCore.Config.Server.Discord
...
function QBCore.Functions.IsPlayerBanned(source)
    ...
    if os.time() < result.expire then
        ...
        return true, 'Sa oled serverist keelustatud:\n' .. result.reason .. '\nKeeld Aegub ' .. timeTable.day .. '/' .. timeTable.month .. '/' .. timeTable.year .. ' ' .. timeTable.hour .. ':' .. timeTable.min .. '\n'
    else
...
function QBCore.Functions.HasItem(source, items, amount)
    if GetResourceState('qb-inventory') == 'missing' then return end
    return exports['lc-inventory']:HasItem(source, items, amount)
end
