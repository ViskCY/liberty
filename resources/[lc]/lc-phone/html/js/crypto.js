...
function UpdateCryptoData(Crypto) {
    ...
    $(".crypto-action-page-wallet").html("Wallet: "+CryptoData.Portfolio+" LCRPBit('s)");
    $(".crypto-walletid").html(CryptoData.WalletId);
    $(".cryptotab-course-list").html("");
    if (CryptoData.History.length > 0) {
        CryptoData.History = CryptoData.History.reverse();
...
$(document).on('click', '#buy-crypto', function(e){
    ...
    if ((Coins !== "") && (Price !== "")) {
        if (QB.Phone.Data.PlayerData.money.bank >= Price) {
            $.post('https://lc-phone/BuyCrypto', JSON.stringify({
                Coins: Coins,
                Price: Price,
            }), function(CryptoData){
                if (CryptoData !== false) {
                    ...
                    QB.Phone.Notifications.Add("fas fa-university", "LCRP Pank", "&#36; "+Price+",- has been withdrawn from your balance!", "#badc58", 2500);
                } else {
...
$(document).on('click', '#sell-crypto', function(e){
    ...
    if ((Coins !== "") && (Price !== "")) {
        if (CryptoData.Portfolio >= parseInt(Coins)) {
            $.post('https://lc-phone/SellCrypto', JSON.stringify({
                Coins: Coins,
                Price: Price,
            }), function(CryptoData){
                if (CryptoData !== false) {
                    ...
                    CloseCryptoPage()
                    QB.Phone.Data.PlayerData.money.bank = parseInt(QB.Phone.Data.PlayerData.money.bank) + parseInt(Price);
                    QB.Phone.Notifications.Add("fas fa-university", "LCRP Pank", "&#36; "+Price+",- has been added to your balance!", "#badc58", 2500);
                } else {
    ...
    if ((Coins !== "") && (WalletId !== "")) {
        if (CryptoData.Portfolio >= Coins) {
            if (WalletId !== CryptoData.WalletId) {
                $.post('https://lc-phone/TransferCrypto', JSON.stringify({
                    Coins: Coins,
                    WalletId: WalletId,
                }), function(CryptoData){
...
$(".crypto-action-page-sell-crypto-input-coins").keyup(function(){
    ...
    $(".crypto-action-page-sell-crypto-input-money").html(MoneyAmount+" Eurot");
});
