...
local function DeleteTarget(id)
    if Config.UseTarget then
        exports['lc-target']:RemoveZone(id)
    else
...
local function RegisterDutyTarget()
    ...
    if Config.UseTarget then
        exports['lc-target']:AddBoxZone(dutyTargetBoxID, coords, 1.5, 1.5, {
        ...
        zone:onPlayerInOut(function (isPointInside)
            if isPointInside then
                exports['lc-core']:DrawText("[E] " .. label, 'left')
            else
...
local function RegisterStashTarget()
    ...
    if Config.UseTarget then
        exports['lc-target']:AddBoxZone(stashTargetBoxID, coords, 1.5, 1.5, {
        ...
        zone:onPlayerInOut(function (isPointInside)
            if isPointInside then
                exports['lc-core']:DrawText(Lang:t('labels.o_stash'), 'left')
            else
                exports['lc-core']:HideText()
            end
...
local function RegisterGarageZone()
    ...
    vehicleZone:onPlayerInOut(function (isPointInside)
        if isPointInside and onDuty then
            ...
            if inVehicle then
                exports['lc-core']:DrawText(Lang:t('labels.h_vehicle'), 'left')
            else
                exports['lc-core']:DrawText(Lang:t('labels.g_vehicle'), 'left')
            end
        else
            exports['lc-core']:HideText()
        end
    ...
    plateZone:onPlayerInOut(function (isPointInside)
        if isPointInside and onDuty then
            if plate.AttachedVehicle then
                exports['lc-core']:DrawText(Lang:t('labels.o_menu'), 'left')
            else
                if IsPedInAnyVehicle(PlayerPedId()) then
                    exports['lc-core']:DrawText(Lang:t('labels.work_v'), 'left')
                end
            end
        else
            exports['lc-core']:HideText()
...
local function OpenMenu()
    ...
    exports['lc-menu']:openMenu(openMenu)
end
...
local function PartMenu(data)
    ...
    exports['lc-menu']:openMenu(TestMenu1)
end
...
local function NoDamage()
    ...
    exports['lc-menu']:openMenu(noDamage)
end
