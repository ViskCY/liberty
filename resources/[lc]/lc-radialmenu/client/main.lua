...
local function controlToggle(bool)
    for i = 1, #controlsToToggle,1 do
        if bool then
            exports['lc-smallresources']:addDisableControls(controlsToToggle[i])
        else
            exports['lc-smallresources']:removeDisableControls(controlsToToggle[i])
        end
    end
end
...
RegisterNetEvent('qb-radialmenu:client:ChangeSeat', function(data)
    ...
    local HasHarnass = exports['lc-smallresources']:HasHarness()
