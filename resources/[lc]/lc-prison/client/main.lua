QBCore = exports['lc-core']:GetCoreObject() -- Used Globally
inJail = false
jailTime = 0
currentJob = nil
CellsBlip = nil
...
RegisterNetEvent('QBCore:Client:OnPlayerLoaded', function()
	...
	exports['lc-target']:AddTargetEntity(freedom_ped, {
		options = {
			{
				type = "client",
	...
	exports['lc-target']:AddTargetEntity(canteen_ped, {
		options = {
			{
				type = "client",
...
RegisterNetEvent('prison:client:Enter', function(time)
	...
	if invokingResource and invokingResource ~= 'lc-policejob' and invokingResource ~= 'lc-ambulancejob' and invokingResource ~= GetCurrentResourceName() then
	...
	if not Config.UseTarget then
		...
		freedom:onPlayerInOut(function(isPointInside)
			...
			if isPointInside then
				...
				exports['lc-core']:DrawText('[E] Check Time', 'left')
			else
				exports['lc-core']:HideText()
			end
		end)
		...
		canteen:onPlayerInOut(function(isPointInside)
			...
			if isPointInside then
				...
				exports['lc-core']:DrawText('[E] Open Canteen', 'left')
			else
				exports['lc-core']:HideText()
			end
		end)
