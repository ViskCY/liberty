    ...
    QBCore.Functions.TriggerCallback('qb-hotdogjob:server:HasMoney', function(HasMoney)
        if HasMoney then
            if Config.UseTarget then
                ...
                exports['lc-target']:AddTargetEntity(StandObject, {
                    options = {
                        {
                            icon = "fas fa-hand",
            ...
            if SellingData.Hotdog ~= nil then
                ...
                if Config.UseTarget then
                    if not zoneMade then
                        zoneMade = true
                        exports['lc-target']:AddEntityZone('sellingDogPed', ped, {
                            name = 'sellingDogPed',
                            debugPoly = false,
                        }, {
                            options = {
                                {
                                    ...
                                    action = function(entity)
                                        ...
                                        SellingData.Hotdog = nil
                                        exports['lc-target']:RemoveZone('sellingDogPed')
                                        zoneMade = false
                                    end,
                                },
                                {
...
CreateThread(function()
    if Config.UseTarget then
        exports['lc-target']:AddBoxZone('hotdog_start', vector3(Config.Locations["take"].coords.x, Config.Locations["take"].coords.y, Config.Locations["take"].coords.z), 1, 1, {
            name = 'hotdog_start',
            debugPoly = false,
        ...
        hotdogStart:onPlayerInOut(function(isPointInside)
            if isPointInside then
                ...
                if PlayerData.job.name == 'hotdog' then
                    if not IsWorking then
                        exports['lc-core']:DrawText(Lang:t("info.start_working"), 'left')
                        CreateThread(function()
                            while inZone do
                                Wait(0)
                                if IsControlJustPressed(0, 38) then
                        ...
                        exports['lc-core']:DrawText(Lang:t("info.stop_working"), 'left')
                        CreateThread(function()
                            while inZone do
                                Wait(0)
                                if IsControlJustPressed(0, 38) then
