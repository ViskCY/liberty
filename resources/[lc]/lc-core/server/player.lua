...
function QBCore.Player.CheckPlayerData(source, PlayerData)
    ...
    PlayerData.items = GetResourceState('lc-inventory') ~= 'missing' and exports['lc-inventory']:LoadInventory(PlayerData.source, PlayerData.citizenid) or {}
...
function QBCore.Player.CreatePlayer(PlayerData, Offline)
    ...
    function self.Functions.SetJob(job, grade)
        ...
        if QBCore.Shared.Jobs[job].grades[grade] then
            ...
            self.PlayerData.job.grade = {}
            self.PlayerData.job.grade.name = 'Pole Auastet'
    ...
    function self.Functions.GetCardSlot(cardNumber, cardType)
        ...
        local slots = exports['lc-inventory']:GetSlotsByItem(self.PlayerData.items, item)
...
function QBCore.Player.Save(source)
    ...
    if PlayerData then
        ...
        if GetResourceState('lc-inventory') ~= 'missing' then exports['lc-inventory']:SaveInventory(source) end
...
function QBCore.Player.SaveOffline(PlayerData)
    if PlayerData then
        ...
        if GetResourceState('lc-inventory') ~= 'missing' then exports['lc-inventory']:SaveInventory(PlayerData, true) end
...
function QBCore.Player.SaveInventory(source)
    if GetResourceState('lc-inventory') == 'missing' then return end
    exports['lc-inventory']:SaveInventory(source, false)
end

function QBCore.Player.SaveOfflineInventory(PlayerData)
    if GetResourceState('lc-inventory') == 'missing' then return end
    exports['lc-inventory']:SaveInventory(PlayerData, true)
end

function QBCore.Player.GetTotalWeight(items)
    if GetResourceState('lc-inventory') == 'missing' then return end
    return exports['lc-inventory']:GetTotalWeight(items)
end

function QBCore.Player.GetSlotsByItem(items, itemName)
    if GetResourceState('lc-inventory') == 'missing' then return end
    return exports['lc-inventory']:GetSlotsByItem(items, itemName)
end

function QBCore.Player.GetFirstSlotByItem(items, itemName)
    if GetResourceState('lc-inventory') == 'missing' then return end
    return exports['lc-inventory']:GetFirstSlotByItem(items, itemName)
end
